...
router.post('/getCoupon',(req,res,next)=>{
    ...
    var query = (connection)=> {
        sql.query({
            connection: connection,
            sql: "SELECT "+
            "a.id,a.task_id,a.user_id,a.coupon_name,a.lower_limit,a.upper_limit,a.coupon_money,DATE_FORMAT(a.end_time,'%Y-%c-%d %h:%i %p') AS end_time,a.use_status,DATE_FORMAT(a.use_time,'%Y-%c-%d') AS use_time,DATE_FORMAT(a.create_time,'%Y-%c-%d') AS create_time,"+
